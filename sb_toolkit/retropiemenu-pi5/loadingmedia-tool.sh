...
if grep -q 'enablevideolaunch="true"' "$RUNONSTART"; then vls=ENABLED; else vls=DISABLED; fi
...
rm /opt/retropie/configs/*/launching.png 2>/dev/null
rm /opt/retropie/configs/*/launching.jpg 2>/dev/null
...
video_screens() {
if grep -q 'enablevideolaunch="true"' "$RUNONSTART"; 
then echo NOW TURNING OFF VIDEO LOADING SCREENS 
sleep 2 
sed -i -E 's|enablevideolaunch="true"|enablevideolaunch="false"|g' $RUNONSTART
else 
echo NOW TURNING ON VIDEO LOADING SCREENS 
sleep 2
sed -i -E 's|enablevideolaunch="false"|enablevideolaunch="true"|g' $RUNONSTART; fi
exit 0
}
