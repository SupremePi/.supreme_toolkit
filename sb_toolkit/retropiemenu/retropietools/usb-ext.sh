...
if [ $? -eq 0 ] ; then
echo -e 'It seems you already have an external drive mapped...\n  *** Only one assigned external drive is supported. *** \nPlease run the "EXTERNAL USB Roms NTFS Setup, EZH!" script again, disable other drive before adding a new drive.'
sleep 10
exit
else
...
cd /etc/samba/
sudo cp /home/pi/.supreme_toolkit/sb_toolkit/_scripts/usb/smb.conf.USB ./
sudo mv smb.conf smb.conf.BAK
sudo ln -s smb.conf.USB smb.conf
cd /etc/profile.d
sudo cp /home/pi/.supreme_toolkit/sb_toolkit/_scripts/usb/10-retropie.sh.USB ./
