...
function main_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "$BACKTITLE" --title " MAIN MENU " \
            ...
            1 "Lightgun Aimtrak Install" \
            2 "Lightgun Aimtrak Config Start" \
            2>&1 > /dev/tty)

        case "$choice" in
            1) lightgun-aimtrak  ;;
            2) lightgun-aimtrak-config  ;;
             *)  break ;;
        esac
    done
}

function lightgun-aimtrak() {
echo "Checking to see if advance-mame is installed if it is not I will install it. This can take a little while to run......."
sleep 5

if [ ! -d "/home/pi/RetroPie/roms/mame-advmame" ]; then
        echo "Advance-Mame not found. Now Installing."
        sleep 10
...
echo ""
echo ""
echo "Installing the needed packages for the gun configuration application."
sleep 3
...
echo ""
echo ""
echo "Downloading gun configuration application. This will appear in your RetroPie menu after install. You will need to calibrate the gun prior to gameplay."
sleep 8
cd
...
function lightgun-aimtrak-config() {
echo Starting...
sleep 3
python -m gunconf
echo Done...
